...
c
c     oct 12, 2016 - SJM: modified to do mean energy calibration rather than mean photon count 
c                         also extrapolated flux to nominal band wavelength using power law 
c               
c       ---------------------------------------------------------------------------
...
c       ===========================================================================
	...
	real lambda_eff0(nmax)
	common /lambda0/ lambda_eff0
	character*6 numz
...
c       count nr of filters to use: nfilt_use
	...
	do k=1,nfilt_use
	   ...
	   lambda_eff0(filt_id_use(k)) = lambda_eff(k_use(k))*1e4 ! save lambda_eff to use later on
	enddo
...
c       Store parameters + magnitudes of each model in output file
	...
	open (29,file=infile,status='old',form='unformatted')
        ...
        do imod=1,50000
c         do imod=1,3  ! just to check if it works
...
c       Effective wavelength of the filter using the model SED:
...
c	Check if filter has been stored already
	...
	write(6,*) 'lamdba_eff (A)', lambda_eff0(i), xeff(i) 	
...
c       for spire filters compute flux below filter using energy weighting and compare to fnu=nu^-1
	if(k.ge.441.and.k.le.449) then 
	   f_mean=TRAPZ1(xf,rfa,m)/TRAPZ1(xf,rfb,m)
	   f_mean=x0*f_mean/2.997925e+18
